...
**実行コンテキストは自分で確定しない。** 親（/test）が対象確定（`/orchestrator`「対象確定（工程共通）」）を済ませたうえで、`phase` / `branch` / `head` / `dirty` の確定値とレポート保存先パスを呼び出しプロンプトで渡してくる。あなたはこれらの値をそのまま frontmatter に書き写すだけで、自分で git を叩いて再確定しない（対象確定の責任者を親に保つため）。`ran_at` だけは書き出し時点で自分が記録する（テスト実行の事実に紐づくため）。
...
先頭に実行コンテキストの frontmatter を置く。`feature` / `phase` / `branch` / `head` / `dirty` は親から渡された確定値をそのまま書き写し、`ran_at` だけは書き出し時点で自分が記録する（値の仕様は `/orchestrator`「対象確定（工程共通）」の「実行コンテキスト frontmatter」参照）。

### test-report（保存先は親から渡されたパス。`test-report.pN.md` または `test-report.md`）
```
---
feature: <feature>
phase: p2                              # 親から受領。フェーズ未確定時は none
branch: <feature>-p2                   # 親から受領。none の場合あり
