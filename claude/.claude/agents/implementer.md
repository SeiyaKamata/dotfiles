...
### 自分で決めた判断（あれば）
- <仕様に書かれておらず自分で決めた点>（該当: <タスク番号 / ファイル>）
```

`blockers` と `自分で決めた判断` は別物として書き分ける。**手が止まったもの**が `blockers`、**止まらず進めたが仕様に根拠が無いもの**（既定値・エラー時の挙動・命名・ライブラリの選び方など）が `自分で決めた判断`。後者は main が完了カードの「要確認」に出して人の目に触れさせるので、黙って埋めずに必ず挙げる。
