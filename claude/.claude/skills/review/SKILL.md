...
## 完了条件
完了条件（done）は **`~/.claude/dev_flow.yml` の `review` 工程を正本とする**（ここでは再掲しない）。
要約すると「仕様・設計との整合とコード品質をレビューし、結果を報告した」状態。
**次工程（next）は dev_flow.yml が正本**：フェーズループ本流では review OK → `/commit`（qa は全フェーズ後に once で 1 回だけ）。単体起動で試すときだけ `/qa` に繋ぐこともあるが、orchestrator 経由の本流の next は commit。NG の戻し先（設計の根本問題は design／それ以外は impl）も dev_flow.yml の `on_ng` と orchestrator の判断による。
