...
書いた seed.md のフルパスと、相手 PJ での拾い方（`cd <依頼先PJ> && claude` → `/spec <機能名>`）を
...
<どの PJ に何を依頼したかを 1 行>
- <主要な結果 最大 3 行>
📄 <依頼先PJ>/.specs/<機能名>/seed.md
▶ OK：依頼先 PJ で /spec <機能名>（`cd <依頼先PJ> && claude` で起動）

完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ 依頼書の受け渡し中断` に差し替え、一言サマリに中断理由（依頼先 PJ を特定できない・`.specs/` に書けないなど）、▶ 行に復帰コマンドを書く（seed.md が未生成なら 📄 行は省略する）。
