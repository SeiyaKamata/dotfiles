...
書き込める場合は続けて `ls <依頼先PJ>/.specs/` で既存 feature 一覧を取得し、Step 3 の採番の入力にする。
...
完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ 依頼書の受け渡し中断` に差し替え、一言サマリに中断理由（依頼先 PJ を特定できない・`.specs/` に書けない・連番の上限に達したなど）、▶ 行に復帰コマンドを書く（seed.md が未生成なら 📄 行は省略する）。
