...
> **人間レビューの依頼タイミングについて**: 人間レビューは `Ready for review` に切り替わってから行う運用のため、自走中（draft のまま）は依頼できない。現状は全 PR が閉じた停止点で人がまとめて Ready にする。PR ごとに Ready にする運用に変える場合は、PR ループ Step 10-4（下記）に切替を差し込む。
...
| `/sync-to-remote`（再呼び出し） | 実在するフェーズブランチのうち最大の `pN` を対象とする。その PR が「CI green かつ未返信の未解決コメントなし」を満たしていれば `<feature>-pN` へ switch して呼ぶ（次フェーズ `p(N+1)` の作成は `/sync-to-remote` 自身が Step 2-6 で行う。orchestrator は先回りして作らない）。満たしていなければ、そのフェーズがまだ完了していないということなので対象外とし、Step 1-3 の追加ルールに従って中断する |
...
`/test` `/review` `/qa` `/fix` の 4 工程は、会話履歴に依存せず単体でコールド起動しても同じ対象に到達できるよう、起動時に対象 feature と実行コンテキストを自力で確定する。その手順の**正本をここに置く**。各工程は自分の節でこの節への参照 1 行を置くだけでよい。
...
対象フェーズ <pN> の PR がまだ CI green + 未返信の未解決コメントなしを満たしていません。/orchestrator watch-ci <feature> pN または /orchestrator resolve-comments <feature> pN から再開してください
...
**完了カードは読み捨てない。** 「要確認」に載る「判断で埋めた点」は、成果物本文には注記が残らないのでカードにしか現れない。これを「工程レビュー（工程共通）」で `stage-reviewer` へのプロンプトに含める。orchestrator は各工程のカードを受け取ったら、遷移先を 1 行で記録しつつ「要確認」の内容は次のレビューまで保持する。
...
- **impl**: 実装ブランチ `<feature>` を 1 本切って全タスクを実装する。大タスクごとに `implementer` を 1 体ずつ依存順に**逐次**配布する（並列配布はしない。同じ作業ツリーへの Write が衝突するため。`impl/SKILL.md` 参照）
- **qa**: commit より前に回す。実装が 1 ブランチで完結しているので、PR を作る前に feature 全体の受け入れを確認できる
- **commit**: 実装ブランチにコミットする（分割はまだ行わない）
- **sync-to-remote**: PR ループの中で起動し、**PR を 1 本だけ**作る。初回に分割を判定し、stacked なら `p1` だけを作って止まる。2 回目以降は次のフェーズブランチで呼ぶ。未コミットがあれば `/commit` を自動で呼ぶ。Notion URL が最初の指示にあれば引数で渡す
- **watch-ci**: PR ループの中で起動する。対象はその周で作った PR。CI green でも draft のまま次へ（Ready 化は停止点で人が判断する）
- **resolve-comments**: `batch` 付きで PR ループの中で起動する。次の PR を作る前に、その PR の未解決コメント（人間 + CodeRabbit）を解消しきる（これが rebase 伝播を避ける肝）

## 工程レビュー（工程共通）

**この節が工程レビューの正本。** `/spec`・`/design`・`/tasks` の完了後、orchestrator が `stage-reviewer` を起動して成果物を検証し、NG ならその工程を再起動して直させる。
...
| 検証条件リスト | 各スキル「書き出し」Step の「書き出しの時点で次を満たす」をそのまま全項目 | 同 | 同 |
...
1. 引数解釈・前提チェック・対象フェーズ決定・ブランチ整合・ディスパッチ（子番号 1-1〜1-5）
   1. Step 1-1: 引数パーサ — `$ARGUMENTS` を `開始工程 / feature / pN` に分解する。判定順:
      ...
      2. 第 1 引数が「工程レジストリ」の工程名（11 語）のいずれかと完全一致する場合
   ...
   3. Step 1-3: 対象フェーズの決定 — 工程レジストリで「対象フェーズ: 不要」の工程（`spec`〜`commit` と `sync-to-remote` の初回）はこの Step をスキップする（第 3 引数が渡されていても無視する）
      1. 「開始工程」節の状態収集コマンドで実在するフェーズブランチと PR を得る（`tasks.md` は読まない。フェーズ構成を持たない）
      2. `pN` が渡されている場合:
      ...
      5. 開始工程が `sync-to-remote` の再呼び出しで、対象フェーズ（実在する最大の `pN`）の PR が「CI green かつ未返信の未解決コメントなし」を満たしていない場合 → そのフェーズはまだ閉じきっていないので `sync-to-remote` を呼ばず、`対象フェーズ <pN> の PR がまだ CI green + 未返信の未解決コメントなしを満たしていません。/orchestrator watch-ci <feature> pN または /orchestrator resolve-comments <feature> pN から再開してください` を示して終了する
...
7. `/review <feature>` を起動。NG は下記「例外処理」（設計起因は `/design`→`/impl`、それ以外は `/impl` に戻す）
   - 判定 OK でも `review.md` の推奨対応に上流 doc の記述修正が挙がっていれば、`/spec`・`/design` を編集モードで再入して記述だけ直す。`/tasks` と実装はやり直さない（実装は正しいため）
8. **受け入れゲート**: `/qa <feature>` を起動する（ブラウザ動作確認。feature 全体の受け入れを 1 回で確認）
   - 全 pass → 次へ
   - fail → `/fix <feature>`（設計起因なら `/design`・`/impl <feature>`）→ `/test <feature>`→`/review <feature>` を通して `/qa` に再合流。qa↔fix が2周しても収束しない → 報告して停止（rebase 伝播が起きない理由は「パイプライン」節の「qa が commit より前にある」参照）
9. `/commit` を起動し、実装ブランチにコミットする（分割はまだ行わない）
10. **PR ループ**: 以下を PR 1 本ずつ回す。**1 周 = 1 PR を CI green + 未返信の未解決コメントなしまで閉じきる**：
    1. `/sync-to-remote` を起動する。初回は分割を判定し（実物の diff とコミット列を見る）、単一なら 1 PR、stacked なら `p1` の draft PR だけを作る。2 周目以降は次のフェーズブランチで呼ばれ、そのフェーズの PR を作る。draft でも CodeRabbit が自動でレビューを開始する
    ...
    5. 未作成の後続フェーズがあるかを確認する（`/sync-to-remote` の完了カードが残り本数を報告している）。あれば今閉じたフェーズのブランチ `<feature>-pN`（今回 10-1〜10-3 で閉じた PR のブランチ）へ `git switch` して **10-1 に戻る**。次のフェーズ `<feature>-p(N+1)` はまだ存在しない — `/sync-to-remote` が再呼び出しと判定した際に Step 2-6 で自分で作る（実装ブランチ `<feature>` を `<feature>-pN` の先端へ載せ直してから分岐する）ので、orchestrator 側で先回りして作ったり switch したりしない。無ければ Step 11 へ
