...
| `/sync-to-remote`（再呼び出し） | 実在するフェーズブランチのうち**最大の `pN`** を対象とする。その PR が「CI green かつ未返信の未解決コメントなし」を満たしていれば `<feature>-pN` へ switch して呼ぶ（次フェーズ `p(N+1)` の作成は `/sync-to-remote` 自身が Step 2-6 で行う。orchestrator は先回りして作らない）。**満たしていなければ、そのフェーズがまだ完了していないということなので対象外とし**、Step 1-3 の追加ルールに従って中断する |
...
10. **PR ループ**: 以下を PR 1 本ずつ回す。**1 周 = 1 PR を CI green + 未返信の未解決コメントなしまで閉じきる**：
    ...
    5. **未作成の後続フェーズがあるか**を確認する（`/sync-to-remote` の完了カードが残り本数を報告している）。あれば**今閉じたフェーズのブランチ `<feature>-pN`**（今回 10-1〜10-3 で閉じた PR のブランチ）へ `git switch` して **10-1 に戻る**。次のフェーズ `<feature>-p(N+1)` はまだ存在しない — `/sync-to-remote` が再呼び出しと判定した際に Step 2-6 で自分で作る（実装ブランチ `<feature>` を `<feature>-pN` の先端へ載せ直してから分岐する）ので、orchestrator 側で先回りして作ったり switch したりしない。無ければ Step 11 へ
...
カードの前後に作業サマリ・所感・補足を足さない。Ready for review への切替・merge は自走で行わず人の判断を待つ。
