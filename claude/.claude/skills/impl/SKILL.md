...
- 引数のいずれかに `auto` を含む場合は自律モード（ブロック抑制）とする。Step 6 で完了カードを出さず簡易ログのみ残す。
...
- 全て満たしたら、末尾「完了カード」を出力して終了する

## 例外処理
- `implementer` が `blockers`（仕様の曖昧さ・設計の穴・大タスクが大きすぎる等）を報告 → メインで判断する。
  - 手動／単体起動時: ユーザーに確認する
  - 自律モード（`auto`）時: 軽微なら追加指示を添えて `implementer` を再配布。仕様・設計・タスク分割の問題なら上流工程（`/spec`・`/design`・`/tasks`）への編集モード再入が要るため、報告して止める（orchestrator の分岐に委ねる）
...
## 完了カード
Step 6 の確認まで終わったら、次の完了カードを**コードフェンスで囲まず**プレーンテキストで出力して終了する。カードの前後に作業サマリ・所感・補足を足さない。次スキルは自動起動せずユーザーの実行を待つ。

- 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**（無ければ行ごと省略）。変更ファイルの一覧や implementer の報告詳細はカードに列挙せず tasks.md と git 差分に寄せる。▶ 行は完了時点で確定した分岐 1 つだけを出す。

✅ フェーズ実装完了
<どのフェーズの何タスクを実装したかを 1 行>
- <主要な結果 最大 3 行>
📄 .specs/<feature>/tasks.md（チェックボックス更新）
▶ OK（テストあり）：/test <feature>
▶ OK（テストなし）：/review <feature>
▶ NG（仕様変更が要る）：変更する工程を編集モードで再入（/spec・/design・/tasks）

完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ フェーズ実装中断` に差し替え、一言サマリに中断理由（未依頼の handoff・blockers・最終テスト失敗など）、▶ 行に復帰コマンドを書く。
