...
## 設計方針: コンテキストフリー
このスキルは会話履歴に依存しない。入力はすべて `.specs/<feature>/` から得る。コールドで `/qa <feature>` を叩いても同じ対象に到達する。`/qa` は **feature 全体の受け入れゲート**であり、`/test` `/review` と異なり**フェーズ引数を取らない**（全フェーズ実装後に最終スタックブランチで 1 回だけ回す位置づけのため、フェーズ単位の工程として扱わない）。

## 入力
- `.specs/<feature>/qa.md`（`tasks` が生成した QA シナリオ）

## 自走モード（`auto` 引数）
`$ARGUMENTS[1]` が `auto` の場合、人間の承認を待たずに結果を報告し、Step 6 の分岐まで自動で進める。
...
### Step 2: 対象確定
`/orchestrator`「対象確定（工程共通）」の手順のうち、`/qa` に該当する 1・2・6（feature の確定・`tasks.md` の存在有無の確認・フェーズ照合は行わない）を実施し、続けて入力 `.specs/<feature>/qa.md` の存在を確認する。`qa.md` が無い場合は「入力 `.specs/<feature>/qa.md` がありません（`/tasks` が生成します）」を示して中断する（末尾「完了カード」の中断カードへ）。
...
- **到達できない（未起動）**:
  ...
  - **auto モード** → 自走を止めないため、自動起動を 1 回だけ試みる:
    ...
    3. 到達できたら Step 4 へ。**起動できない／到達しない／別 worktree 使用中で `-loop` が失敗したとき**は、qa を回さず `## QA結果: BLOCKED`（起動不可）として qa-report.md に記録し、オーケストレータ（または `/fix`）に返して停止する。人間承認は待たない。
...
完了カードの主要な結果に 1 行載せる。提案を出したときはカードの外に別文で書かず、主要な結果の 1 行
（例: `- 💡 提案 1 件発行: <claim>`）として載せる。
...
## 完了カード
Step 5〜6 で判定が確定したら、次の完了カードを**コードフェンスで囲まず**プレーンテキストで出力して終了する。カードの前後に作業サマリ・所感・補足を足さない。次スキルは自動起動せずユーザーの実行を待つ。
