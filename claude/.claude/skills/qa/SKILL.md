...
工程を終える際、この工程が本来 catch すべきだった**汎用の**逸脱に気づき、かつ一行の `claim` を書けるなら、`/propose` で提案を1件出す（PJ 固有の事実は該当 PJ の CLAUDE.md/memory へ）。自動では出さず提案に留める。提案を出したときはカードの外に別文で書かず、主要な結果の 1 行（例: `- 💡 提案 1 件発行: <claim>`）として載せる。
