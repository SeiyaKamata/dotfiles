...
完了カードの共通仕様（CLAUDE.md）に従う。見出しは `### QA 完了 — PASS` / 中断時（**BLOCKED を含む**）`### QA 中断`。
...
- QA 完了: 見出しの末尾に判定（PASS / FAIL）を付ける。**検証を実施して確定した判定だけがこちら側**で、検証そのものを実施できなかった BLOCKED は中断カード側に出す。失敗シナリオの内訳・スクショパスは qa-report.md に寄せ、カードには列挙しない。生成物は 1 件 1 行（qa-report.md 未生成ならその行を省略）。
- 要確認: シナリオの手順が曖昧で `qa-browser` が解釈で補った点、環境依存で結果が変わりうる点を挙げる。**Step 6 の停止に失敗したときは必ずここに書く**。環境が上がったまま残るので、次の作業に影響する。
