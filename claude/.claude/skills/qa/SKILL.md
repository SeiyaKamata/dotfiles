...
## 環境の扱い
**環境は落ちている前提で始め、開始時点の状態に関係なく検証が終わったら常に片付ける。** 検証には自分の worktree のコードが載った環境が要るので、`/qa` は起動ステップ（Step 3）と停止ステップ（Step 6）を自分で持つ。「自分が起動した分だけ停止する」という条件はメモリ上の判定（起動フラグ）に依存し、追跡を一度誤ると環境が起動しっぱなしのまま残る。`swws stop` は未起動の状態に対しても安全に呼べるため、無条件に呼んで構わない。
...
到達できたら Step 4 へ。**起動できない／到達しない／`-loop` が空かずに失敗したとき**は `BLOCKED`（起動不可）として扱う。qa は回さず、**Step 6 で停止してから**中断する。人間承認は待たない。
...
FAIL の原因調査で環境が要るなら `/fix` 側で起動し直す。これで次に `/qa` を回すときも「環境は落ちている」前提が成り立つ。
...
- QA 中断: 一言サマリに中断理由。`qa.md` が無い・環境を起動できず検証を実施できないなどが該当する。成果物が未生成なら生成物の行は省略する。**中断時も Step 6 は実行済み**（`/swws stop` を実行してから中断する）。
