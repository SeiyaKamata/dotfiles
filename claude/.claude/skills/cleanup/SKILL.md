...
## モード
このスキルは**取り消せない削除（`.specs/<feature>`）を伴うので確認を残す**。着地が確認できない（PR が未マージ・デフォルトブランチへ未取り込み）とき、または `## マージ後の検証` に未チェックの `V<n>` が残っているときは必ず人に確認し、勝手に消さない。起動も人が行う。
...
**未消費のマージ後検証を確認する:** `.specs/<feature>/tasks.md` の `## マージ後の検証` は**人が PR マージ後に実施してチェックを付ける**節なので、`.specs/<feature>` を消すとチェックリストごと消える。着地を確認したら、この節の**未チェック（`[ ]`）の `V<n>`** を数える：
...
- 要確認: Step 3-2 で列挙した**残存フェーズブランチ**（cleanup は消さないので、消すかは人が判断する）と、Step 2 で未チェックだったまま削除を了承された **`V<n>`** を出す。`.specs` を消すとチェックリストが残らないので、内容をここに引き写す。どちらも無ければブロックごと省略する。
- 次の一手: `/clear` はハーネスコマンドだが同じ形式で書き、**自動実行せず**ユーザーの実行を待つ。

**中断時**: ヘッダを `⚠ 後片付け中断` に差し替え、一言サマリに中断理由（着地していない PR がある・未チェックの `V<n>` を先に検証する・`proposals` が渡されたなど）、次の一手に復帰コマンドを書く。`V<n>` 待ちで止めた場合は該当項目を挙げ、検証後に `/cleanup <feature>` を再実行する道を出す。
