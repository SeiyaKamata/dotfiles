...
開始工程を `/orch <stage> <feature> [<phase>]` で任意の位置から指定できる（指定工程より前は実行しない）。`<stage>` にはスラッシュなしの工程名を渡す（例: `design`）。省略時は従来どおり spec 開始。`watch-ci` / `resolve-comments` / `sync-to-remote` の再呼び出しのように対象フェーズが必要な工程では、`<phase>` に `pN`（例: `p1`）を渡す。省略された場合は Step 1-2 で人に直接確認する（推定はしない）。
...
4. **Step 1-4: ディスパッチ** — 工程レジストリの「開始 Step」へジャンプし、開始工程をそのまま起動する。ジャンプ後の挙動はそれぞれの Step の記述そのままとし、以下を追加ルールとする:
