...
書式・テンプレートへの適合は各工程の完了ゲートが担うので、レビュアーには渡さない。
...
- レビュアーの「要確認」は判定を止めない。
  判定が OK なら次工程へ進み、要確認の内容は完了カードの「要確認」と同じ扱いで次のレビューまで保持し、停止点の報告に含める
- レビューは 1 体で観点 A の上流との突合・観点 B の中身の正当性の 2 つを見る。
  観点を分けて複数体を並列起動しない
...
- 裏取りが `未検証` のまま、または分類の確信が持てないと明記された件は `保留` にする。
  `/sync` が保留の返信と issues.md 記録を行うので、スレッドは未返信のまま残らず、拾うかは人が issues.md で判断する
