...
- **フェーズは orch が扱う概念ではない。** どのフェーズブランチ・PR が対象かは `sync-to-remote` / `watch-ci` / `resolve-comments` が自分で解決する（各 SKILL.md 参照）。orch は `<feature>` を渡すだけで、フェーズ番号を人に聞いたり推定したりしない。

パイプライン全体の流れ（各工程の順序・次工程・戻し先・承認ゲート・scope）は「## 進め方」「## 例外処理」を正本とする。停止点の条件は「## 完了条件」を正本とする。

## 進め方

**完了カードは人向けの区切りではなく工程間の引き継ぎ情報。** 各工程がカードを出力しても、そこで応答を終えず、同じ応答内で「完了後」に定めた次のアクションを続けて実行する。次のアクションには `stage-reviewer` の起動や次工程の起動も含む。ユーザーの応答を待って止まるのは「## 完了条件」を満たしたときと「## 例外処理」の停止条件に該当したときだけ。

開始工程を `/orch <stage> <feature>` で任意の位置から指定できる（指定工程より前は実行しない）。`<stage>` にはスラッシュなしの工程名を渡す（例: `design`）。省略時は従来どおり spec 開始。
...
**前提成果物は事前チェックしない。** 開始工程をそのまま起動し、不足していれば起動した skill 自身がそれを検知して案内する（各 skill は単体起動でも同じ案内を出す設計になっている）。orch はその案内に従うだけでよい（Step 1-3 参照）。この理由により工程レジストリは前提成果物を列に持たない。

| 開始工程 | 開始 Step | 起動コマンド |
|---|---|---|
| `spec` | Step 2 | `/spec <feature>` |
| `design` | Step 3 | `/design <feature>` |
| `tasks` | Step 4 | `/tasks <feature>` |
| `impl` | Step 5 | `/impl <feature>` |
| `test` | Step 6 | `/test <feature>` |
| `review` | Step 7 | `/review <feature>` |
| `qa` | Step 8 | `/qa <feature>` |
| `commit` | Step 9 | `/commit`（feature を渡さない） |
| `sync-to-remote` | Step 10-1 | `/sync-to-remote <feature>`（対象ブランチの解決は sync-to-remote 自身が行う） |
| `watch-ci` | Step 10-2 | `/watch-ci <feature>`（対象 PR の解決は watch-ci 自身が行う） |
| `resolve-comments` | Step 10-3 | `/resolve-comments <feature> batch`（対象 PR の解決は resolve-comments 自身が行う） |

### Step 1: 引数解釈・ブランチ整合・ディスパッチ

1. **Step 1-1: 引数パーサ** — `$ARGUMENTS` を `開始工程 / feature` に分解する。判定順:
   1. 引数が 0 個 → `使い方: /orch [<stage>] <feature>` を表示して終了
   2. 第 1 引数が工程レジストリの工程名（11 語）のいずれかと完全一致する場合
      - 第 2 引数（feature）が無い → `使い方: /orch [<stage>] <feature>` を表示して終了
...
2. **Step 1-2: ブランチ整合** — `impl`〜`commit` から開始する場合、実装ブランチ `<feature>` が存在すればそこへ `git switch` する。存在しなければ切り替えない。`sync-to-remote` / `watch-ci` / `resolve-comments` から開始する場合は switch しない（**対象ブランチ・対象 PR の解決は起動先の skill 自身が行う**。各 SKILL.md 参照）。未コミットの変更があって切り替えられない場合は報告して停止する（stash などの作業ツリー操作はしない）
3. **Step 1-3: ディスパッチ** — 工程レジストリの「開始 Step」へジャンプし、開始工程をそのまま起動する。ジャンプ後の挙動はそれぞれの Step の記述そのままとし、以下を追加ルールとする:
   1. 開始 Step より前の Step は実行しない。したがって開始工程が design 以降のときは Step 2（`/spec`）を実行せず、既存の requirements.md を確定済みの成果物として扱う
   2. **前提不足のリカバリ**: 起動した skill が前提の不足（対象ブランチ・対象 PR が無い場合、対象 PR がまだ CI green + 未返信の未解決コメントなしを満たしていない場合を含む）を理由に中断したら、その skill 自身の案内に従って不足を生む工程を実行し、完了したら元の開始工程を再実行する。案内された工程を実行してもなお中断する場合は報告して停止する。**事前チェックはしない** — 起動してから skill 自身の判断に従う
...
**対象ブランチ**（再呼び出しのとき）: `sync-to-remote` 自身が `<feature>` から解決して switch する。次フェーズ `p(N+1)` の作成も `/sync-to-remote` 自身が Step 2-6 で行うため、orch は先回りして作らない。
...
**完了後**:
- 未返信の未解決コメントなし かつ 未作成の後続フェーズがある → `/sync-to-remote <feature>` に戻る（ブランチの解決・switch は sync-to-remote 自身が行う。次のフェーズ `p(N+1)` の作成も sync-to-remote 自身が行うため、orch 側で先回りして作らない）
...
- **前提成果物が不足しており、案内された工程を実行してもなお起動した skill が中断する** → 中断理由と復帰コマンドを示して終了（Step 1-3）
- **実装ブランチへ切り替えられない**（未コミットの変更がある）→ 報告して停止し、stash などの作業ツリー操作は行わない（Step 1-2）
