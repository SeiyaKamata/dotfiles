...
## モード
**本番への修正なので自走させない。** 人が起動し、下記 2 箇所で確認を取る。
...
修正は**症状を止める最小限**にとどめる（本番へ当てるため）。リファクタ・周辺の改善は混ぜず、気づいたことは `CLAUDE.md`「訂正の扱い」に従って切り出す。複数ステップに分かれる場合は適宜進捗を共有する。
...
- やったこと: 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**（起点 tag・release / work ブランチ名）。PR 本文・diff の詳細は転記しない。`bug-report.md` は入力なので 📄 行は出さない。🔗 行は **1 PR 1 行**で 2 行出す（行数上限は主要な結果にだけ課す）。
...
**中断時**: ヘッダを `⚠ hotfix 中断` に差し替え、一言サマリに中断理由、次の一手に復帰コマンドを書く（作成できた PR があれば 🔗 行は残す）。
