...
2つのPR URLを末尾の `## 完了カード` に畳んで報告する（カードの外に別文で書かない）。
...
## 完了カード
2 本の PR 作成が済んだら、次の完了カードを**コードフェンスで囲まず**プレーンテキストで出力して終了する。カードの前後に作業サマリ・所感・補足を足さない。次スキルは自動起動せずユーザーの実行を待つ。

- 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**（起点 tag・release / work ブランチ名を載せる）。PR 本文・diff の詳細はカードに転記しない。
- 🔗 行は **1 PR 1 行**で release 宛・main 宛の 2 行を出す（行数上限は主要な結果にだけ課す）。
...
完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ hotfix 中断` に差し替え、一言サマリに中断理由（未コミットの変更がある・tag が存在しない・ブランチ名衝突など）、▶ 行に復帰コマンドを書く（作成できた PR があれば 🔗 行は残す）。
