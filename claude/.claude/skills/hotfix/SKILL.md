...
完了カードの共通仕様（CLAUDE.md）に従う。見出しは `### hotfix PR 作成完了` / 中断時 `### hotfix 中断`。
...
- やったこと: PR 本文・diff の詳細は転記しない。`bug-report.md` は入力なので生成物の行は出さない。生成物は **1 PR 1 行**で 2 行出す。
- 要確認: **本番に当てる修正**なので、確認しきれていない点を必ず挙げる（ローカルで再現できなかった条件・影響範囲の見立てなど）。**Step 4-1 の照合で tag と `bug-report.md` に差分があった場合は必ずここに書く。**

**中断時**: 中断理由と次の一手の対応は「エラー処理」に従う（作成できた PR があれば生成物の行は残す）。**原因が定まらないまま実装へ進む道は出さない。**
