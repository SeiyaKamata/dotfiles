...
## 次ステップ提示
現在の状態の要約に続けて、次の定型ブロックを**コードフェンスで囲まず**プレーンテキストで出力する。推定フェーズが一意に確定するなら該当コマンドを 1 つ提示する。

```
────────────────────────────────
✅ 再開ポイント特定
▶ 次のステップ
   <推定した再開コマンド（例: /impl <feature>）>
   理由: <なぜそのフェーズと判断したか1行で>
────────────────────────────────
```

複数のフェーズにまたがって曖昧な場合は、`▶ 次のステップ` を候補列挙に差し替え、2〜3 件をそれぞれの条件付きで提示して人間に選ばせる。
