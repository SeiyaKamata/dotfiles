...
**中断時**（発行しなかった場合を含む）: ヘッダを `### 提案発行中断` に差し替え、一言サマリに理由（`claim` を一行で書けない・プロダクト側のスコープ切り分け・symlink 未設置など）、次の一手に代わりの行き先（`- 棚上げする: /spinoff`、`- 別 PJ セッションと会話して詰める`）を書く（ファイル未生成なので生成物の行は省略する）。
