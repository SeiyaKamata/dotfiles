...
- **PJ 固有の事実**（特定 PJ でのみ成立する前提・設定・運用ルール）→ キューには発行しない。
  `/spinoff`（その PJ で直せるもの）または `/handoff`（別 PJ 側の変更が要るもの）で**改善提案に
  切り出す**よう案内し、ここで終了する。恒久ルール（`CLAUDE.md`）へ勝手に追記しない — 反映するかは
  人が判断する。
...
- 気づきが PJ 固有の事実と判定された場合は、キューへ発行せず `/spinoff`・`/handoff` での改善提案に
  回す旨を案内して終了する（振り分け）。
...
発行しなかった／完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ 提案発行中断` に差し替え、一言サマリに理由（`claim` を一行で書けない・PJ 固有の事実だった・symlink 未設置など）、▶ 行に代わりの行き先（`/spinoff`・`/handoff` で改善提案に切り出す など）を書く（ファイル未生成なので 📄 行は省略する）。
