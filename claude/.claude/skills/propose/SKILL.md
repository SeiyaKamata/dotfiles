...
発行しなかった／完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ 提案発行中断` に差し替え、一言サマリに理由（`claim` を一行で書けない・PJ 固有の事実だった・symlink 未設置など）、▶ 行に代わりの行き先（該当 PJ の `CLAUDE.md` / memory へ記録 など）を書く（ファイル未生成なので 📄 行は省略する）。
