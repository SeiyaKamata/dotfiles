...
完了カードの共通仕様（CLAUDE.md）に従う。見出しは `### CI 監視完了` / 中断時 `### CI 監視中断`。
...
- やったこと: 失敗ジョブのログ本体は載せず要点 1 行に畳む。生成物の行は対象 PR を **1 本 1 行**（stacked なら監視した全 PR）。
...
**中断時**: 中断理由は PR 未作成・`--watch` タイムアウト・権限エラーなど。次の一手の復帰コマンドは PR 未作成なら `- PR を作る: /sync-to-remote`。判定が出ていないまま次工程へ進む道は出さない。
