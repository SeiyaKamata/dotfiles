...
`--watch` で完了までブロッキング監視する。表示が止まる区間に入るので、開始時に「出力規律」の 1 行を出す（1 PR につき 1 回）：
...
- やったこと: 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**（無ければ行ごと省略）。失敗ジョブのログ本体は載せず要点 1 行に畳む。🔗 行は対象 PR を **1 本 1 行**（stacked なら監視した全 PR。行数上限は主要な結果にだけ課す）。
