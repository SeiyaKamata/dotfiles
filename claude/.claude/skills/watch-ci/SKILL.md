...
#### 5-4: 再発しそうな失敗は改善提案に切り出す

CI失敗の原因が **その PJ で繰り返しハマる構造的なもの**（例: この PJ の CI は import 順 lint で
必ず落ちる、特定の env を入れ忘れると必ず E2E が落ちる、CI 専用の前提手順がある）だと判断したら、
**記録ではなく改善提案として切り出す**（毎回ハマる構造ごと直す提案にする）。恒久ルール
（`CLAUDE.md`）へ勝手に追記しない — 反映するかは人が判断する。

- 切り出し先は成果物の種類で選ぶ: 一行のルールに落ちるなら `/propose`、手順・構成の変更が要るなら
  `/spinoff`（この PJ で直せる。`.specs/<新feature>/seed.md`）・`/handoff`（別 PJ 側の変更が要る。
  dotfiles のスキル・パイプライン側の穴も dotfiles 宛）
- 本スキルからは**自動起動しない**。切り出し先を完了カードの主要な結果に 1 行載せ、実行は人に委ねる
