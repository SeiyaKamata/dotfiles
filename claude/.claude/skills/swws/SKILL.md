...
## モード
`auto` は持たない。**別 worktree を奪う判断は人がする**（自走で奪うと他セッションの作業を壊す）。

exit 2（別 worktree 使用中）のときだけ確認する。**破壊的操作の確認**であって成果物の内容確認ではないので、他工程の「聞かない」方針の対象外。
...
要確認:
- ⚠️ 混在: <同一プロジェクトに複数 worktree が同居している旨と推奨手順>
...
- やったこと: 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**。切替先 worktree のパスとプロファイル（`status` なら使用中の worktree）を載せ、`docker compose` の出力ログは転記しない。swws は**ファイル成果物を持たない**ので 📄 行は出さない。
