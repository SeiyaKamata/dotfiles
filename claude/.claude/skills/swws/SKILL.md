...
### Step 5: 結果を報告する
どの worktree のどのプロファイルに切り替えたか（または解放したか）を、末尾の `## 完了カード` に畳んで報告する（カードの外に別文で書かない）。
...
## 完了カード
切り替え（または `status` / `stop`）が済んだら、次の完了カードを**コードフェンスで囲まず**プレーンテキストで出力して終了する。カードの前後に作業サマリ・所感・補足を足さない。

- swws は**ファイル成果物を持たない**ので 📄 行は出さない。切替先 worktree のパスとプロファイル（`status` なら使用中の worktree）を主要な結果に載せる。
- 一言サマリは 1 行。主要な結果は `- ` の箇条書きで**最大 3 行**。`docker compose` の出力ログはカードに転記しない。
- Step 4 の別 worktree 使用中の確認フローは**対話ゲート**なので、ナレーション制限の対象外として従来どおり出す（そのうえで確認後の結果をカードで畳む）。

✅ worktree 切り替え完了
<どのプロジェクトをどのプロファイルで起動したかを 1 行>
- <切替先 worktree・プロファイルなど 最大 3 行>
▶ OK：この worktree で作業を続行

完了条件を満たせずに終了するときは、同じ構成でヘッダを `⚠ worktree 切り替え中断` に差し替え、一言サマリに中断理由、▶ 行に判断先を書く（別 worktree 使用中なら `▶ NG（別 worktree 使用中）：SWWS_FORCE=1 swws <profile> で強制切替を判断`）。
