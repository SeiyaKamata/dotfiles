...
完了カードの共通仕様（CLAUDE.md）に従う。見出しは `### worktree 切り替え完了` /
中断時 `### worktree 切り替え中断`。
...
切替先 worktree のパスとプロファイル、`status` なら使用中の worktree を載せ、
`docker compose` の出力ログは転記しない。swws は**ファイル成果物を持たない**ので生成物の行は出さない。
要確認は、混在を検知したとき、強制切替（`SWWS_FORCE=1`）で**別セッションを止めたとき**に出す。

3-2 の確認フローは**対話ゲート**なので、ナレーション制限の対象外として従来どおり出す（そのうえで確認後の結果をカードに畳む）。

**中断時**: 次の一手に判断先を書く（別 worktree 使用中なら `- 強制切替を判断: SWWS_FORCE=1 swws <profile>`）。
