...
`swws` 本体が自分以外の worktree が稼働中なら exit 2 で止まる。空き・自分の worktree のときはそのまま起動する。
exit コードで分岐する：
...
- **1** → `swws -loop <profile>` で空くまで待つ。
  ...
  空かずに失敗、exit 2、したら 3-2 に戻って再確認する
- **2** → 何もせず終了する
...
3-2 の確認フローは対話ゲートなので、ナレーション制限の対象外として従来どおり出す。そのうえで確認後の結果をカードに畳む。
